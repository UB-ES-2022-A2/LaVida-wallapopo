...
def test_login_post():
    ...
    wrong_email_format = requests.post(url + "login", json)
    ...
    account_not_exist = requests.post(url + "login", json)
    ...
    blank_password = requests.post(url + "login", json)
    ...
    wrong_password = requests.post(url + "login", json)
...
def test_logout_post():
    ...
    r = requests.post(url + "logout/pepe433@gmail.com", auth=auth)
    ...
    r = requests.post(url + "logout/pepe432@gmail.com", auth=wrong_auth)
    assert r.status_code == 400
    assert r.json() == {'message': "Bad authorization user"}
    """

    r = requests.post(url + "logout/pepe432@gmail.com", auth=auth)
